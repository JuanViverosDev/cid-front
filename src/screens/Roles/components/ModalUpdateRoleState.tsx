...
import { updateRoleState } from '../../../services/roles.service';

export const ModalUpdateRoleState = ({ openModal, setOpenModal, roleSelected, setRoleSaved, roleSaved }: any) => {
  const { callEndpoint } = useFetchAndLoad();

  const handleSubmit = async () => {
    const body = {
      roleState: !roleSelected.state,
    };
    ...
    try {
      const response = await callEndpoint(updateRoleState(roleSelected, body));
      if (response.success) {
        setOpenModal(false);
        //Reload list with newRole
        setRoleSaved(!roleSaved);
        showToast('success', 'State updated successfully', 'Please check the state filter to find this role.');
      }
    } catch (error) {
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              ...
              <div className="sm:flex sm:items-start">
                <div className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-blue-100 sm:mx-0 sm:h-10 sm:w-10">
                  <ExclamationIcon
                    className={
                      'h-6 w-6 ' + (roleSelected.state ? ' text-cidTertiaryLightPurple' : ' text-cidPrimaryGreen')
                    }
                    aria-hidden="true"
                  />
                </div>
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      {roleSelected.state
                        ? 'Are you sure you want to block this role?'
                        : 'Are you sure you want to unlock this role?'}
                    </p>
                  </div>
                </div>
              </div>
              <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                ...
                >
                  {roleSelected.state ? 'Block Role' : 'Unlock Role'}
                </button>
                <button
                  type="button"
