...
export const ModalUpdateSystemState = ({ openModal, setOpenModal, system, setSystemSaved, systemSaved }: any) => {
  ...
  /* const handleSubmit = async () => {
    ...
    try {
      const response = await callEndpoint(updateBankOfficeState(system, body));
      ...
      if (response.success) {
        ...
        showToast(
          ...
          "Please check the state filter to find this Office."
        );
      } else {
        showToast(
          "error",
      ...
      showToast(
        "error",
        "Please, contact with support department",
        "An error occurred updating the state."
      );
    }
  }; */
  return (
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              ...
              <div className="sm:flex sm:items-start">
                <div className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-white sm:mx-0 sm:h-10 sm:w-10">
                  <ExclamationIcon
                    className={'h-6 w-6 ' + (system.state ? ' text-cidTertiaryLightPurple' : ' text-cidPrimaryGreen')}
                    aria-hidden="true"
                  />
                </div>
                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                  <Dialog.Title as="h3" className="text-lg leading-6 font-medium text-gray-900">
                    Cambiar estado de este sistema
                  </Dialog.Title>

                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      {system.state
                        ? '¿Estás seguro(a) de que desea bloquear este sistema?'
                        : '¿Estás seguro(a) de que desea desbloquear este sistema?'}
                    </p>
                  </div>
                </div>
              </div>
              <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                ...
                >
                  {system.state ? 'Bloquear este sistema' : 'Desbloquear este sistema'}
                </button>
                <button
                  type="button"
