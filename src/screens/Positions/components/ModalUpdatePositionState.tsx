...
import { updatePositionsState } from "../../../services/positions.service";

export const ModalUpdatePositionState = ({
  openModal,
  setOpenModal,
  positionSelected,
  setPositionSaved,
  ...
  const changeState = async () => {
    const body = {
      positionState: !positionSelected.positionState,
    };
    ...
    try {
      const response = await callEndpoint(
        updatePositionsState(positionSelected, body)
      );
      ...
      showToast(
        "error",
        "Por favor comuníquese con soporte técnico",
        "Ocurrio un error actualizando el cargo"
      );
    }
  };
  return (
              ...
              <div className='sm:flex sm:items-start'>
                <div className='mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-white sm:mx-0 sm:h-10 sm:w-10'>
                  <ExclamationIcon
                    className={
                      "h-6 w-6 " +
                      (positionSelected.positionState
                ...
                <div className='mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left'>
                  ...
                  <div className='mt-2'>
                    <p className='text-sm text-gray-500'>
                      {positionSelected.positionState
                        ? "¿Estás seguro(a) de bloquear este cargo?"
                        : "¿Estás seguro(a) de desbloquear este cargo?"}
                    </p>
                  </div>
                </div>
              </div>
