...
import { updateModuleState } from '../../../../services/menu.service';

export const ModalUpdateModuleState = ({
  openModal,
  setOpenModal,
  moduleSelected,
  setModuleSaved,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await callEndpoint(updateModuleState(moduleSelected, body));
      if (response.success) {
        setOpenModal(false);
        //Reload list with newModule
        setModuleSaved(!moduleSaved);
        showToast('success', 'State updated successfully', 'Please check the state filter to find this module.');
      }
    } catch (error) {
          ...
          >
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              ...
              <div className="sm:flex sm:items-start">
                <div
                  className={
                    ...
                    (moduleSelected.state ? ' bg-red-50' : ' bg-green-50')
                  }
                >
                  <ExclamationIcon
                    className={
                      'h-6 w-6 ' +
                      (moduleSelected.state ? ' text-cidTertiaryLightPurple' : ' text-cidPrimaryGreen')
                    }
                    aria-hidden="true"
                  />
                </div>
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      {moduleSelected.state
                        ? 'Are you sure you want to block this module?'
                        : 'Are you sure you want to unlock this module?'}
                    </p>
                  </div>
                </div>
              </div>
              <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                ...
                >
                  {moduleSelected.state ? 'Block Module' : 'Unlock Module'}
                </button>
                <button
                  type="button"
