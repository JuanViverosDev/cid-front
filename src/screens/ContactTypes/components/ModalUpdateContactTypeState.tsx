...
import { updateContactTypeState } from "../../../services/comunicationType.service";

export const ModalUpdateContactTypeState = ({
  openModal,
  setOpenModal,
  contactTypeSelected,
  setContactTypeSaved,
  ...
  const changeState = async () => {
    const body = {
      communicationState: !contactTypeSelected.communicationState,
    };
    ...
    try {
      const response = await callEndpoint(
        updateContactTypeState(contactTypeSelected, body)
      );
              ...
              <div className='sm:flex sm:items-start'>
                <div className='mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-white sm:mx-0 sm:h-10 sm:w-10'>
                  <ExclamationIcon
                    className={
                      "h-6 w-6 " +
                      (contactTypeSelected.communicationState
                ...
                <div className='mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left'>
                  ...
                  >
                    Cambiar estado del tipo de contacto
                  </Dialog.Title>

                  <div className='mt-2'>
                    <p className='text-sm text-gray-500'>
                      {contactTypeSelected.communicationState
                        ? "¿Estás seguro(a) de bloquear este tipo de contacto?"
                        : "¿Estás seguro(a) de desbloquear este tipo de contacto?"}
                    </p>
                  </div>
                </div>
              </div>
