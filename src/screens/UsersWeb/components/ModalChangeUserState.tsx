...
import { updateWebUserState } from '../../../services/webUsers.service';

export const ModalChangeUserState = ({ openModal, setOpenModal, userSelected, setUserSaved, userSaved }: any) => {
  const { callEndpoint } = useFetchAndLoad();

  const handleSubmit = async () => {
    const body = {
      userState: !userSelected.userState,
    };
    ...
    try {
      const response = await callEndpoint(updateWebUserState(userSelected, body));

      if (response.success) {
        setOpenModal(false);
        //Reload list with new user
        setUserSaved(!userSaved);
        showToast('success', 'State updated successfully', 'Please check the state filter to find this user.');
      } else {
        showToast('error', 'Please, contact with support department', response.message);
      }
    } catch (error) {
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              ...
              <div className="sm:flex sm:items-start">
                <div className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-white sm:mx-0 sm:h-10 sm:w-10">
                  <ExclamationIcon
                    className={
                      'h-6 w-6 ' + (userSelected.userState ? ' text-cidTertiaryLightPurple' : ' text-cidPrimaryGreen')
                    }
                    aria-hidden="true"
                  />
                </div>
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      {userSelected.userState
                        ? '¿Estás seguro(a) que deseas bloquear a este agente?'
                        : '¿Estás seguro(a) que deseas desbloquear a este agente?'}
                    </p>
                  </div>
                </div>
              </div>
              <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                ...
                >
                  {userSelected.userState ? 'Bloquear agente' : 'Desbloquear Agente'}
                </button>
                <button
                  type="button"
