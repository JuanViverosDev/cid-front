...
import { updateRequestTypeState } from "../../../services/requestTypes.service";

export const ModalUpdateRequestTypeState = ({
  openModal,
  setOpenModal,
  requestType,
  setRequestTypeSaved,
  ...
  const handleSubmit = async () => {
    const body = {
      typeReqState: !requestType.typeReqState,
    };
    debugger;

    try {
      const response = await callEndpoint(
        updateRequestTypeState(requestType, body)
      );

      if (response.success) {
        setOpenModal(false);
        //Reload list with new request type
        setRequestTypeSaved(!requestTypeSaved);
        showToast(
          "success",
          "Estado actualizado correctamente",
          "Por favor revise el filtro de estado para ver este cambio"
        );
      } else {
        showToast(
          "error",
            ...
            <div className='inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6'>
              ...
              <div className='sm:flex sm:items-start'>
                ...
                <div className='mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left'>
                  ...
                  >
                    Cambiar estado del tipo de solicitud
                  </Dialog.Title>

                  <div className='mt-2'>
                    <p className='text-sm text-gray-500'>
                      {requestType.typeReqState
                        ? "¿Estás seguro(a) de que desea bloquear este tipo de solicitud?"
                        : "¿Estás seguro(a) de que desea desbloquear este tipo de solicitud?"}
                    </p>
                  </div>
                </div>
              </div>
              <div className='mt-5 sm:mt-4 sm:flex sm:flex-row-reverse'>
                ...
                >
                  {requestType.typeReqState
                    ? "Bloquear este Tipo de Solicitud"
                    : "Desbloquear este Tipo de Solicitud"}
                </button>
                <button
                  type='button'
